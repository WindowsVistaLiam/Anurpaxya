...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('heal')
    .setDescription('Retirer de la souillure à un joueur')
    ...
    .addNumberOption(option =>
      option
        .setName('montant')
        .setDescription('Montant de souillure à retirer')
        .setRequired(true)
        .setMinValue(0.1)
        .setMaxValue(100)
    )
  ...
  async execute(interaction) {
    ...
    const newSouillure = Math.max(0, Number((oldSouillure - amount).toFixed(2)));
    ...
    await interaction.reply({
      content:
        `✨ Souillure réduite pour **${profile.nomPrenom || targetUser.username}** ` +
        `(**slot ${slot}**).\n` +
